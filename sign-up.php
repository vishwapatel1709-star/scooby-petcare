...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($password !== $confirmpassword) {
        ...
        $check = mysqli_prepare($con, "SELECT id FROM signup WHERE email = ?");
        ...
        if (mysqli_stmt_num_rows($check) > 0) {
            ...
            $stmt = mysqli_prepare($con, "INSERT INTO signup (firstname, lastname, email, phone_no, address, password, confirmpassword) VALUES (?, ?, ?, ?, ?, ?, ?)");
            mysqli_stmt_bind_param($stmt, "sssssss", $firstname, $lastname, $email, $phone, $address, $password, $confirmpassword);
            ...
            if (mysqli_stmt_execute($stmt)) {
                ...
                $signup_error = "Registration failed: " . mysqli_error($con);
            }
...
<!-- Mirrored from demo.egenslab.com/html/scooby/preview/sign-up.php by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 18 Jan 2024 09:23:41 GMT -->
<head>
<meta charset="UTF-8">
...
<h6><a href="tel:+1(541)754-3010">+91-9825268887</a></h6>
</div>
</div>
...
<?php if(!empty($signup_error)) echo '<div class="alert alert-danger">' . htmlspecialchars($signup_error) . '</div>'; ?>
...
<input type="email" name="email" placeholder="email" pattern="(^[^\s@]+@[^\s@]+\.[^\s@]+$)" required>
</div>
</div>
<div class="col-md-12">
<div class="form-inner">
<label>Enter Your Phone no *</label>
<input type="tel" id="phone no" name="phone_no" placeholder="phone no" pattern="[0-9]{10}" required>
</div>
</div>
<div class="col-md-12">
<div class="form-inner">
...
<input type="password" name="password" id="password" placeholder="Create A Password" placeholder="password" id="password" minlength="8" pattern="(?=.\d)(?=.[a-z])(?=.[A-Z])(?=.[!@#$%^&()-_+=])[a-zA-Z\d!@#$%^&()-_+=]{8,}" title="Password must be at least 8 characters long and include at least one uppercase letter, one lowercase letter, one digit, and one special character." required/>
...
<input type="password" name="confirmpassword" id="password2" placeholder="Confirm Password" laceholder="password" id="password" minlength="8" pattern="(?=.\d)(?=.[a-z])(?=.[A-Z])(?=.[!@#$%^&()-_+=])[a-zA-Z\d!@#$%^&()-_+=]{8,}" title="Password must be at least 8 characters long and include at least one uppercase letter, one lowercase letter, one digit, and one special character." required/>
