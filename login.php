...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $stmt = mysqli_prepare($con, "SELECT * FROM signup WHERE email = ?");
    ...
    if ($row = mysqli_fetch_assoc($result)) {
        // Compare password (plain text as stored in DB)
...
<!-- Mirrored from demo.egenslab.com/html/scooby/preview/login.php by HTTrack Website Copier/3.x [XR&CO'2014], Thu, 18 Jan 2024 09:23:41 GMT -->
<head>
<meta charset="UTF-8">
...
<h3>Log In</h3>
<p>New Member? <a href="sign-up.php">signup here</a></p>
</div>
<form method="POST">
<div class="row">
<div class="col-12">
...
if(isset($_GET['registered']) && $_GET['registered']==1) echo '<div class="alert alert-success">Registration successful! Please log in.</div>';
?>
...
<input type="password" name="password" id="password" placeholder="Password" id="password" minlength="8" pattern="(?=.\d)(?=.[a-z])(?=.[A-Z])(?=.[!@#$%^&()-_+=])[a-zA-Z\d!@#$%^&()-_+=]{8,}" title="Password must be at least 8 characters long and include at least one uppercase letter, one lowercase letter, one digit, and one special character." required>
...
<a href="forgotpswd.php" class="forgot-pass">Forgotten Password</a>
</div>
</div>
</div>
